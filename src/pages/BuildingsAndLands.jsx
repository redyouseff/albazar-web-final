...
import getAllLandingHook from '../Hook/listing/getAllLandingHook';
...
// Constants from AddBuildingsAndLands
const propertyTypes = ['زراعية', 'تجارية', 'صناعية', 'سكنية'];
const regulationOptions = [
  { value: 'داخل التنظيم', label: 'داخل التنظيم' },
  { value: 'خارج التنظيم', label: 'خارج التنظيم' }
];
...
const currencyOptions = [
  { value: 'ليره', label: 'ليرة سورية', flag: '🇸🇾' },
  { value: 'دولار', label: 'دولار أمريكي', flag: '🇺🇸' }
];
const paymentOptions = [
  { value: 'كاش', label: 'كاش' },
  { value: 'تقسيط', label: 'تقسيط' },
  { value: 'كاش أو تقسيط', label: 'كاش أو تقسيط' }
];
const publishedViaOptions = [
  { value: 'المالك', label: 'المالك' },
...
const BuildingsAndLands = () => {
  // Filter state
  const [filters, setFilters] = useState({
    city: '',                  // Changed from governorate to city
    regulationStatus: '',      // Matches API parameter
    publishedVia: '',         // Changed from advertiser to publishedVia
    'property type': '',      // Exact match with API parameter
    area: '',
    amenities: '',
    currency: '',
    price: '',
    'payment method': '',     // Exact match with API parameter
    'listing status': '',     // Exact match with API parameter
    negotiable: '' ,
    post:true,
              // Changed from isNegotiable
  });

  const { loading, error, listings, page, limit, totalPages, handlePageChange, handleLimitChange } = getAllLandingHook(filters);
      ...
      <div className={`${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}> 
                  ...
                  <div className={`container mx-auto py-4 relative z-10 px-2 font-[Noor] text-right ${isDarkMode ? 'text-gray-100' : ''}`}>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-3" dir="rtl">
                      ...
                      <Select value={filters['property type']} onValueChange={(value) => handleFilterChange('property type', value)}>
                        <SelectTrigger className={`rounded-full w-full h-[42px] font-medium font-[Noor] border-0 transition-all ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-white text-gray-900'} focus:ring-0 focus:ring-offset-0 focus:outline-none outline-none`}>
                          <SelectValue placeholder="النوع" className={`text-sm ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`} />
                        </SelectTrigger>
                        <SelectContent className={`font-[Noor] rounded-xl border-0 shadow-lg p-2 mt-2 ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-white text-gray-900'}`}>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-3" dir="rtl">
                      ...
                      <Select value={filters.amenities} onValueChange={(value) => handleFilterChange('amenities', value)}>
                        ...
                        <SelectContent className={`font-[Noor] rounded-xl border-0 shadow-lg p-2 mt-2 ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-white text-gray-900'}`}>
                          <SelectItem value="pool" className="text-sm font-medium focus:outline-none rounded-lg hover:bg-black/5 cursor-pointer py-2 px-3 text-right">حمام سباحة</SelectItem>
                          <SelectItem value="garden" className="text-sm font-medium focus:outline-none rounded-lg hover:bg-black/5 cursor-pointer py-2 px-3 text-right">حديقة</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                      ...
                      <Select value={filters.currency} onValueChange={(value) => handleFilterChange('currency', value)}>
                        ...
                        <SelectContent className={`font-[Noor] rounded-xl border-0 shadow-lg p-2 mt-2 ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-white text-gray-900'}`}>
                          {currencyOptions.map(currency => (
                            <SelectItem key={currency.value} value={currency.value} className="text-sm font-medium focus:outline-none rounded-lg hover:bg-black/5 cursor-pointer py-2 px-3 text-right">
                    ...
                    <div className="flex flex-wrap gap-2 mb-3" dir="rtl">
                      <button
                        ...
                        className={`flex items-center gap-2 rounded-full px-3 h-[42px] font-medium text-sm cursor-pointer border-2 transition-all ${filters.negotiable === 'yes' ? 'border-yellow-500' : isDarkMode ? 'border-gray-500' : 'border-black'} ${isDarkMode ? 'bg-gray-900 text-gray-100 hover:bg-gray-700' : 'bg-white/0 text-black hover:bg-black/5'}`}
                      >
        ...
        <div className={`${isDarkMode ? 'bg-gray-800/90' : 'bg-[#FFED00]'} relative px-2 sm:px-4 md:px-8 w-[95%] mx-auto rounded-[20px] sm:rounded-[30px] my-4 sm:my-8 border ${isDarkMode ? 'border-gray-700' : 'border-yellow-200'}`}>
          <div className={`container mx-auto py-4 sm:py-6 relative z-10 px-2 sm:px-4 font-[Noor] text-right ${isDarkMode ? 'text-gray-100' : ''}`}>
            {/* First Row */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mb-3" dir="rtl">
              <Select value={filters.city} onValueChange={(value) => handleFilterChange('city', value)}>
                <SelectTrigger className={`rounded-full w-full h-[42px] font-medium font-[Noor] border-0 transition-all ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-white text-gray-900'} focus:ring-0 focus:ring-offset-0 focus:outline-none outline-none`}>
                  <SelectValue placeholder="المحافظة" className={`text-sm ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`} />
                </SelectTrigger>
              ...
              <Select value={filters.regulationStatus} onValueChange={(value) => handleFilterChange('regulationStatus', value)}>
                ...
                <SelectContent className={`font-[Noor] rounded-xl border-0 shadow-lg p-2 mt-2 ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-white text-gray-900'}`}>
                  {regulationOptions.map(opt => (
                    <SelectItem key={opt.value} value={opt.value} className="text-sm font-medium focus:outline-none rounded-lg hover:bg-black/5 cursor-pointer py-2 px-3">
                      {opt.label}
                    </SelectItem>
                  ))}
                </SelectContent>
              ...
              <Select value={filters['property type']} onValueChange={(value) => handleFilterChange('property type', value)}>
                ...
                <SelectContent className={`font-[Noor] rounded-xl border-0 shadow-lg p-2 mt-2 ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-white text-gray-900'}`}>
                  {propertyTypes.map(type => (
                    <SelectItem key={type} value={type} className="text-sm font-medium focus:outline-none rounded-lg hover:bg-black/5 cursor-pointer py-2 px-3">
                      {type}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            {/* Third Row */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 mb-3" dir="rtl">
              <Input
                type="number"
                placeholder="السعر"
                value={filters.price}
            ...
            {/* Checkboxes */}
            <div className="flex flex-wrap gap-2 sm:gap-4 mb-3" dir="rtl">
              <button
                onClick={() => handleCheckboxChange('negotiable')}
        ...
        <div className="container mx-auto py-8 sm:py-12 px-4">
          {loading ? (
            ...
            <div className="text-center py-16 sm:py-32 px-4">
              ...
              <h3 className={`text-2xl sm:text-3xl font-bold mb-4 sm:mb-6 font-[Noor] ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>لا توجد مباني أو أراضي</h3>
              <p className={`text-base sm:text-lg ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} font-[Noor] max-w-md mx-auto`}>لم يتم العثور على أي مباني أو أراضي تطابق معايير البحث الخاصة بك</p>
            </div>
          ) : (
            <>
        ...
        {!loading && processedListings?.length > 0 && (
          <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-gray-50'} py-12 sm:py-16`}>
            <div className="container mx-auto px-4">
              <h2 className={`text-2xl sm:text-3xl font-bold text-right mb-8 sm:mb-10 font-[Noor] ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>الأكثر بحثا في الأراضي والمباني</h2>
...
export default BuildingsAndLands; 